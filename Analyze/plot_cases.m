...
addpath(genpath('A_functions')); cols;

% Figure 1
% c_subject = [123124, 3, 602]; %contrast definition

% Figure 4
% c_subject = [509, 505]; %tu vs hyperintensity

% %Figure 5
% c_subject = [5, 403, 2, 123123]; %interpretation

% Others
% c_subject = [1,6,602,601,401];
% c_subject = [2, 501, 502, 503, 504]; %risk of being missed, false negative
% c_subject = [505,508]; %risk of being missed
% c_subject = [506,507,401]; %risk of being included, false positive
% c_subject = [1,6,602,601,401]; %ste vs lte
% c_subject = [2,3,4,5]; %clearer contrast to white matter part 1
% c_subject = [7,8,9,501]; %clearer contrast to white matter part 2
% c_subject = [403]; %rest not used
% c_subject = [101,604,331]; % texture differences
% c_subject = [603,201]; %WM tracts
% c_subject = [2, 501, 502, 503, 504]; %risk of being missed, false negative
...
for c_exp = 1:numel(c_subject)
    ...
    contrast_scale_list{c_exp} = ...
        ...
        [(sss{20}) (sss{21})],... %ste 2000 %%%!!!
        [(sss{20}) (sss{21})],...
        [(sss{20}) (sss{21})]}; %%%%%!!!!
    
    subject_name = strcat('Glioma_project_',ss{c_exp,1});
    disp(subject_name)
...
for c_con = 1:n_contrast
    for c_exp = 1:numel(c_subject)
        ...
        if strcmp(ss{c_exp,1},'170') && c_con == 2
        ...
        if c_con == 7 %adjust STE
            ...
            tmp1 = tmp1 * lte_bright(c_exp)/ste_bright(c_exp);
        end
